...
def obtener_extractos_Integra(cuenta, token, desde, hasta, limit=10000):
    """
    Obtiene extractos de la cuenta usando variables de entorno en Azure
    """
    client_id    = os.environ.get("INTEGRA_CLIENTID")
    customer_id  = os.environ.get("INTEGRA_USER")

    if not client_id or not customer_id:
        raise ValueError("No se encontraron las variables de entorno necesarias para Integra")
